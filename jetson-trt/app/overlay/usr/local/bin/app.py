#!/usr/bin/env python3
"""Jetson TensorRT GPU inference dashboard.

Builds a TensorRT engine from a bundled ONNX model, runs continuous inference
on the integrated Tegra GPU, and serves live throughput/latency stats.

Stack exercised end to end:
    nvgpu + nvidia.ko + libcuda (Jetson BSP driver)
      -> CUDA runtime (cuda-cudart / cuda-libraries from the feed)
        -> TensorRT engine build (JIT for this Jetson's GPU, e.g. Orin sm_87)
          -> execution with cuda-python device buffers

Unlike the x86 RTX reference this imports the STANDARD `tensorrt` module
(python3-tensorrt), not `tensorrt_rtx`. Everything else -- engine build, IO
tensor binding, execute_async_v3 -- is the same TensorRT 10 API.
...
def query_gpu_name():
    ...
    On Tegra `nvidia-smi` is often absent or only partially wired, so ask CUDA
    directly (works on the integrated GPU) and fall back to nvidia-smi, then to
    the device-tree board model.
    """
    try:
        props = cuda_check(cudart.cudaGetDeviceProperties(0))
    ...
    try:
        ...
        return "unknown (integrated Tegra GPU)"


def build_engine():
    """Return a serialized TensorRT engine, using an on-disk cache."""
    ...
    # FP16 is the Jetson sweet spot; enable it when the GPU supports it.
...
<title>Jetson TensorRT Inference</title><meta http-equiv=refresh content=2>
...
<h1>Jetson TensorRT GPU Inference</h1><div class=g>%s</div>
