...
# marshalling for TensorRT execution). numpy comes from the feed
# (python3-numpy) and tensorrt from the feed (python3-tensorrt).
#
# NOTE: cuda-python must be ABI-compatible with the on-device CUDA runtime
# (CUDA 13.x on JetPack 7). `cuda-python` on PyPI ships CUDA-12 and CUDA-13
# aligned builds; pin (e.g. `cuda-python==13.*`) if pip resolves the wrong major.
set -euo pipefail
